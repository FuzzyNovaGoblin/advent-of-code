...
impl CordPoint {
    pub fn new(x: i128, y: i128) -> Self { Self { x, y } }
}
...
impl<T> From<Vec<Vec<T>>> for PointMap<T>
where
    T: Default + Display + Ord,
{
    fn from(from_data: Vec<Vec<T>>) -> Self {
        let mut ret_val = PointMap::default();
        for (i, d) in from_data.into_iter().enumerate() {
            for (j, d) in d.into_iter().enumerate() {
                ret_val[(i, j)] = d;
            }
        }
        ret_val
    ...
    T: Display + Default,
{
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        let write_h_line = |f: &mut std::fmt::Formatter<'_>| -> std::fmt::Result {
            write!(f, "{}", "*".repeat(self.points.len() * (max_size + 1) + 4))
        };
...
//     fn index(&self, cord: CordPoint) -> &Self::Output {
//         if cord.x >= 0 && cord.y >= 0 {
//             self.index((cord.x as usize, cord.y as usize))
//         } else {
//             panic!("cant index unmutable with negetive numbers");
//         }
//     }
// }
...
//     T: Display + Default + PartialOrd,
// {
//     fn index_mut(&mut self, cord: CordPoint) -> &mut Self::Output {
//         if cord.x >= 0 && cord.y >= 0 {
//             return self.index_mut((cord.x as usize, cord.y as usize));
//         }

//         let (_, height) = self.get_dimentions();

//         while cord.x < 0 {
//             self.points.insert(0, Default::default());
//             self.points.first_mut().into_iter().for_each(|column| {
//                 while column.len() < height {
//                     column.push(Default::default());
//                 }
//             });
...
//         }
//         while cord.y < 0 {
//             for column in self.points.iter_mut() {
//                 column.insert(0, Default::default());
//                 cord.y += 1;
//             }
//         }
//         self.index_mut((0_usize, 0_usize))
//     }
// }
